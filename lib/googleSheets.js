...
async function fetchSheetData(sheetName) {
  try {
    ...
    console.log(`Fetching cell data for ${sheetName}...`);

    let response;
    try {
      // Try to fetch with hyperlink support
      ...
      console.warn(`Failed to fetch with hyperlinks for ${sheetName}, trying without: ${apiError.message}`);
      // Fallback: fetch without hyperlink fields
      response = await sheets.spreadsheets.get({
        spreadsheetId,
        ranges: [`${sheetName}!A3:I1000`],
    ...
    if (!sheetData) {
      console.warn(`No data returned for ${sheetName}`);
      return [];
    }

    const rowData = sheetData.rowData || [];
    console.log(`Processing ${rowData.length} rows from ${sheetName}`);
    ...
    for (let i = 0; i < rowData.length; i++) {
      ...
      try {
        ...
        if (inventoryIdCell?.textFormatRuns && Array.isArray(inventoryIdCell.textFormatRuns)) {
        ...
        // Also check if the entire cell has a hyperlink
        if (!inventoryIdLink && inventoryIdCell?.hyperlink) {
          inventoryIdLink = inventoryIdCell.hyperlink;
        }
      } catch (linkError) {
        // Log but don't fail if hyperlink extraction fails
        console.warn(`Failed to extract hyperlink for ${values[0]?.userEnteredValue?.stringValue}:`, linkError);
      }
